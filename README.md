...
A build tool for OpenSCAD projects. Manage dependencies, configure assemblies, and build STL files from a single CLI.

## Prerequisites

- Python 3.10+
- Git (for downloading dependencies)
- OpenSCAD or OpenSCAD-Nightly (for building STL files)
...
| Command | Description |
|---------|-------------|
| `buildscad init [--name NAME]` | Initialize a new project. Creates `buildscad.properties`, `deps.json`, `scad/`, `stl/`, and `.gitignore`. If `--name` is provided, a new directory is created with that name. If omitted, the current working directory is initialized in place and its name is used as the project name. |
...
| `buildscad build` | Build all configured assemblies into STL files in `stl/`. Automatically pulls dependencies first. |
| `buildscad clean [--keep-stl]` | Remove the `dependencies/` folder and all `.stl` files. `--keep-stl` preserves STL output. |
